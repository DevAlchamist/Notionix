...
import {
  ...
  sanitizeSummaryText,
} from "./summaryderive.js";

export type Platform = "ChatGPT" | "Claude" | "Gemini" | "Bing" | "Grok" | "Unknown";
...
function detectPlatform(url: string): Platform {
  try {
    ...
    if (hostname.includes("bing.com") || hostname.includes("copilot.microsoft.com")) return "Bing";
...
async function getStoredAuthToken(): Promise<string | null> {
  return new Promise((resolve) => {
    chrome.storage.sync.get(["authToken"], (result: { authToken?: string }) => {
...
async function sendTabMessage(tabId: number | null | undefined, payload: Record<string, unknown>): Promise<void> {
...
async function broadcastRuntimeMessage(payload: Record<string, unknown>): Promise<void> {
  await new Promise<void>((resolve) => {
...
async function saveSummaryDirectly(message: any): Promise<{ ok: true } | { ok: false; reason: string }> {
...
async function ensureContentScriptForTab(tabId: number, url: string): Promise<void> {
...
function injectIntoActiveTab(windowId?: number) {
...
chrome.tabs.onActivated.addListener((activeInfo) => {
  chrome.tabs.get(activeInfo.tabId, (tab) => {
    if (chrome.runtime.lastError || !tab?.id || !tab.url) return;
    void ensureContentScriptForTab(tab.id, tab.url);
  });
});
