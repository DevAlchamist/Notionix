...
export async function runExtensionGoogleOAuth(): Promise<{
  ...
  const urlBody = (await urlRes.json()) as { url?: string; error?: string };
  if (urlBody.error) {
  ...
  if (!code || !state) {
    throw new Error(
      "Missing authorization code from Google. In Google Cloud Console, add this redirect URI for your OAuth client: " +
        redirectUri,
    );
  }
  ...
  const data = (await exchangeRes.json()) as { token?: string; user?: AuthUser };
  if (!data.token) {
