 ...
 *
 * Run from web/: npm run migrate:summary-visibility
 * Requires MONGO_URI in environment (e.g. web/.env.local).
 */
import mongoose from "mongoose";
import { Summary } from "../src/server/models/Summary";

async function main() {
  const uri = process.env.MONGO_URI?.trim();
  if (!uri) {
    console.error("MONGO_URI is not set. Add it to web/.env.local");
    process.exit(1);
  }
